...
func _process(delta: float) -> void:
	...
	if curState == lightStates.WAITING:
		if lightTimer <= 0:
			lightTimer = randomize_timer(passiveRange)
			position.x = randomize_timer(appearX)
			set_state(lightStates.GREEN, Color("55b328"))
			playerCaught = false
	elif curState == lightStates.GREEN:
		if lightTimer <= 0:
			#curState = lightStates.YELLOW
			lightTimer = warnLength
			set_state(lightStates.YELLOW, Color("d9b914ff"), 88)
	elif curState == lightStates.YELLOW:
		#light.color = Color("d9b914ff")
		if lightTimer <= 0:
			lightTimer = 0.4
			set_state(lightStates.ATTACKING, Color("eb0024"), 100)
	elif curState == lightStates.ATTACKING:
		if lightTimer <= 0:
			anims.play("strike")
			if playerReference.inputHandler.get_movement_input() && !playerCaught:
				playerCaught = true
				lightStrikes += 1
				if lightStrikes == 3:
					playerReference.health = -1
	elif curState == lightStates.FALLBACK:
		playerCaught = false
